...
if [ "$DOWNLINK" = "true" ]; then
    ...
    for i in $(seq 1 $ITNUM); do
        (
            (sleep $SLEEP_DUR && irtt client --tripm=$TRIPM -i $INTERVAL -f $INTERVAL_OFFSET -g $EXP_NAME/edge -l $LENGTH -m $MULT -d $DUR -o d --outdir=$CL_OUT_DIR --local=:$CL_PORT $SERVER_IP) &
